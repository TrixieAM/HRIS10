...
app.post("/api/payroll", (req, res) => {
  const {
      employeeNumber, name, position, rateNbc188, nbc594, increment, grossSalary, abs, h, m, s,
  ...
  try {
    for (const record of attendanceData) {
      ...
      await new Promise((resolve, reject) => {
        const query = `
          INSERT INTO payroll (employeeNumber, startDate, endDate, h, m, s)
          VALUES (?, ?, ?, ?, ?, ?)
        `;
        db.query(query, [employeeNumber, startDate, endDate, h, m, s], (err) => {
          if (err) reject(err);
          else resolve();
        });
      });
    ...
    console.error("Error inserting to payroll:", err);
